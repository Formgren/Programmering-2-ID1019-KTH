...
defmodule Evaluate do
  ...
  def test() do
    expr0 = {:add, {:add, {:mul, {:num, 3}, {:var, :x}}, {:div, {:num,7},{:num, 2}}},{:mul, {:div, {:var, :y}, {:num, 3}}, {:num, 5}}}
    expr5 = {:add, {:div, {:num, 3}, {:num, 4}}, {:div, {:num, 3}, {:num, 4}}}
    #expr0 = {:add, {:sub, {:mul, {:num, 3}, {:var, :x}}, {:q, 7, 2}}, {:var, :y}}
    expr1 =  {:div, {:add, {:num, 10}, {:var, :x}}, {:num, 5}}
    expr2 = {:div, {:num, 5}, {:var, :y}}
    expr3 = {:div, {:var, :z}, {:num, 0}}
    env = %{x: 5, y: 15, z: 20}

    # (eval(expr1, env))
    # {eval(expr1, env), eval(expr2, env), eval(expr3, env)}
    eval(expr1, env)
  end
  ...
  def eval({:add, n1, n2}, env) do
    eval(add(eval(n1, env), eval(n2, env)),env) end
  def eval({:sub, n1, n2}, env) do
    sub(eval(n1, env), eval(n2,env)) end
  def eval({:mul, n1, n2}, env) do
    mul(eval(n1, env), eval(n2, env)) end
  def eval({:div, n1, n2}, env) do
    divi(eval(n1, env), eval(n2,env)) end

  def add({:num, n1}, {:num, n2}) do {:num, (n1 + n2)} end
  def add({:q, q1, q2}, {:num, n}) do
    divi({:num, (q1+(q2*n))}, {:num, q2}) end
  def add({:num, n}, {:q, q1, q2}) do
    divi({:num, (q1+(q2*n))},{:num, q2}) end
  def add({:q, q1, q2}, {:q, q3, q4}) do
    {:q, ((q1*q4)+(q2*q3)),(q2*q4)} end

  def sub({:num, n1},{:num, n2}) do n1 - n2 end

  def sub({:q, q1, q2}, {:num, n}) do
    divi({:num, q1-(q2*n)},{:num, q2}) end

  def sub({:num, n}, {:q, q1, q2}) do
    divi({:num, (q2*n)-q1},{:num, q2}) end
  ...
  def mul({:num, 0}, _) do 1 end
  def mul(_, {:num, 0} ) do 1 end
